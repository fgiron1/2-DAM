...
//            }
    ...
    public static function deleteBooksBy($num_paginas_max){

        //Ejecutamos la siguiente query

        //DELETE FROM libros
        //WHERE numpag <= $num_paginas_max

        $dao = DatabaseModel::getInstance();
        $connection = $dao->getConnection();

        $query = "DELETE FROM ". ConsLibrosModel::TABLE_NAME . " WHERE "
            .ConsLibrosModel::PAGS . " <= ?";

        $prep_query = $connection->prepare($query);

        $prep_query->bind_param($num_paginas_max);
