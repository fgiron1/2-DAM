...
BEGIN
	...
	(SELECT I.Fecha FROM inserted AS I 
	 ...
	 WHERE I.Fecha NOT BETWEEN P.AperturaApuestas AND P.CierreApuestas)
		...
		BEGIN
			ROLLBACK
			RAISERROR(N'Las apuestas para este partido no están disponibles', 12, 50001);
		END
...
BEGIN
	...
	BEGIN
		ROLLBACK
		RAISERROR(N'La apuesta realizada supera el límite.', 12, 50002);
	END
END
GO
...
BEGIN
	...
	BEGIN
		ROLLBACK
		RAISERROR(N'La apuesta realizada supera el límite.', 12, 50003);
	END
END
GO
...
BEGIN
    ...
    BEGIN
        ROLLBACK
		RAISERROR(N'La apuesta realizada supera el límite.', 12, 50004);
    END
END
GO
